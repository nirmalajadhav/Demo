<?php
	...
	if (file_exists(dirname(dirname(dirname(__FILE__))) . DIRECTORY_SEPARATOR . 'cache' . DIRECTORY_SEPARATOR . 'script-'.CDN_VERSION.'.js')) {
		if (!empty($_SERVER['HTTP_IF_MODIFIED_SINCE']) && strtotime($_SERVER['HTTP_IF_MODIFIED_SINCE']) == filemtime(dirname(dirname(dirname(__FILE__))) . DIRECTORY_SEPARATOR . 'cache' . DIRECTORY_SEPARATOR . 'script-'.CDN_VERSION.'.js')) {
			header("HTTP/1.1 304 Not Modified");
			exit ;
		}
	
		@readfile(dirname(dirname(dirname(__FILE__))) . DIRECTORY_SEPARATOR . 'cache' . DIRECTORY_SEPARATOR . 'script-'.CDN_VERSION.'.js');
		...
		include_once (dirname(dirname(dirname(__FILE__))) . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'js' . DIRECTORY_SEPARATOR . 'jquery-ui-1.10.4.custom.min.js');
		...
		$fp = @fopen(dirname(dirname(dirname(__FILE__))) . DIRECTORY_SEPARATOR . 'cache' . DIRECTORY_SEPARATOR . 'script-'.CDN_VERSION.'.js', 'w');
		@fwrite($fp, $js);
		@fclose($fp);
	}
	
	$lastModified = filemtime(dirname(dirname(dirname(__FILE__))) . DIRECTORY_SEPARATOR . 'cache' . DIRECTORY_SEPARATOR . 'script-'.CDN_VERSION.'.js');
